...
$$
\mathcal{B}^{*}=\{i:\mathrm{column}(i)=3\},
\qquad
\mathcal{I}^{*}=\{i:\mathrm{column}(i)=4\}.
$$
...
For the cluster-structured DGP, the primary boundary analysis compares the matched boundary and interior sets $\mathcal{B}^{*}$ and $\mathcal{I}^{*}$ defined in Section 3.6.
...
=
\frac{1}{|\mathcal{B}^{*}|}
\sum_{i\in\mathcal{B}^{*}}d_{ibm}
-
\frac{1}{|\mathcal{I}^{*}|}
\sum_{i\in\mathcal{I}^{*}}d_{ibm}.
$$

In the core $5\times5$ cluster DGP, $|\mathcal{B}^{*}|=|\mathcal{I}^{*}|=5$, and boundary and interior nodes are matched by lattice row.
